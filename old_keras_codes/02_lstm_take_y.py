...
model.fit(X_train_rec, Y_train_rec, nb_epoch=100, batch_size=1000)
#score = model.evaluate(X_test, y_test)

norm_predictions = model.predict(X_test_rec)
predictions = (norm_predictions * train_y_std) + train_y_mean;

output_save = np.concatenate((test_y.reshape(test_y.shape[0],1)[0+time_delay:predictions.shape[0]+time_delay], predictions), axis=1)
